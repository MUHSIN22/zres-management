...
function TakeAwayOrders({ clickedSubCategory, selectedNav }) {
  ...
  useEffect(() => {
    if (location.pathname === "/mainPage/delivery-manager/take-away-order") {
      setPath("waiting");
    } else {
      setPath("collected");
    }
      ...
      <div className="body__top__headder__section">
        <div className="hedder__category">
          <div className="header__items">
            <Link className={path === "waiting" && "active"} to="">
              <h5>Waiting for collection</h5>
            </Link>
            <Link className={path === "collected" && "active"} to="collected">
              <h5>Collected</h5>
            </Link>
          </div>
